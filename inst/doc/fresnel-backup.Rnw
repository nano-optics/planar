...
rewritten as,
\begin{equation}
  1 - r =\sqrt{\frac{\mu_1\varepsilon_2}{\mu_2\varepsilon_1}}\cdot t.
\label{eq:rt3}
\end{equation}
Note that in the case of normal incidence, we can combine equation~\ref{eq:rt1} and equation~\ref{eq:rt3}, 
\[
t_{12} = \frac{2\sqrt{\mu_2\varepsilon_1}}{\sqrt{\mu_2\varepsilon_1}+\sqrt{\mu_1\varepsilon_2}}, \qquad r_{12}=\frac{\sqrt{\mu_2\varepsilon_1}-\sqrt{\mu_1\varepsilon_2}}{\sqrt{\mu_1\varepsilon_2}+\sqrt{\mu_2\varepsilon_1}}.
\]
...
\begin{equation*} \left\{
\begin{aligned}
1 - r &=\sqrt{\frac{\mu_1\varepsilon_2}{\mu_2\varepsilon_1}}t\\
1 + r &=\frac{n_1k_{z2}}{n_2k_{z1}}t
\end{aligned}\right.
\end{equation*}
% 
...
\section{Reflectivity of a multilayer structure}
\label{sec:stack}
...
\begin{itemize}
\item Compute and store for later use all single-interface coefficients, namely $r_{01}$, $r_{12}$, $r_{23}$, $r_{34}$
\item Combine the previous coefficients in Eq.~\ref{eq:fresnelSlab} to compute the reflectivity of the last layer, i.e. $r_{24} = \frac{r_{23}+r_{34}\exp(2i k_{z3}d_3)}{1+r_{23}r_{34}\exp(2i k_{z3}d_3)}$. This value can be stored in a temporary variable $r_{\text{tmp}}$
\item Go up, combining one interface at a time, $r_{\text{tmp}}=r_{14} = \frac{r_{12}+r_{24}\exp(2i k_{z2}d_2)}{1+r_{12}r_{24}\exp(2i k_{z2}d_2)}$
\item the combined reflectivity of the multilayer is obtained when one has reached the top interface, $r_{\text{tmp}} = r_{04} = \frac{r_{01}+r_{14}\exp(2i k_{z1}d_1)}{1+r_{01}r_{14}\exp(2i k_{z1}d_1)}$
\end{itemize}
