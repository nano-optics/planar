...
combined <- data.frame(theta = c(stack$theta, stack$theta + pi) * 180/pi,
                      parallel = c(M$Mr.par[[1]], M$Ml.par[[1]]),
                      perpendicular = c(M$Mr.perp[[1]], M$Ml.perp[[1]]),
                      side = gl(2, length(stack$theta),
                                labels=c("front", "back")))
...
theta <- c(front$theta + pi/2, back$theta + 3*pi/2) * 180 / pi
