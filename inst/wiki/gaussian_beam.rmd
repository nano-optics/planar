...
The detrimental effect of the beam size on the field enhancement is quite dramatic: as soon as the spread of incident angles causes the coupling to SPPs to deteriorate. Here we simulate the LFIEF for silver, with a beam waist varying from 1000 microns to 1 micron.
...
simul <- function(w0=10){
  w0 <- w0*1e3
  xyz <- as.matrix(expand.grid(x=seq(-5*w0, 5*w0+5000,length=100), y=0, z=0))
...
all$waist <- factor(all$w0, levels = c(1e3, 5e2, 1e2, 50, 10, 5, 1))
ggplot(all, aes(x/w0/1000, field))+
  geom_line() + facet_grid(waist~., scales="free", labeller=label_both) +
